	...
	<div class="doTask">
		...
		<ul class="list-shop">
			<li v-for="(item, index) in shopList">
				...
				<p>{{item.createTime}}</p>
				<p>
					<router-link class="button" :to="{path: '/main/taskDetail', query: {businessId: item.businessId}}">
						查看
				    </router-link>
				</p>
			</li>
...
export default {
  ...
  methods: {
    getShopList (page) {
      ...
      this.axios.get('/bdm/api/history-tasks', {params: {page: this.pager.page, size: 15, sort: 'endTime,desc'}}).then(res => {
